  ...
  const updateChannelSubject = (channelId, subjectUserId) => {
    setChannels((channels) =>
      channels.map((channel) =>
        channel.channel_id === channelId ? { ...channel, subject_user_id: subjectUserId } : channel
      )
    );
  };
  ...
  return html`
    <div className="mt-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
      <div className="mb-3 flex items-start justify-between gap-3">
        ...
        html`<span
          className="shrink-0 rounded-md border border-white/[0.08] px-2 py-1 font-mono text-[10px] text-iron-500"
        >
          ${channelsQuery.data.team_id}
        </span>`}
      </div>
      ...
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center">
        ...
        >
          ${!hasRoutableSubjects && html`<option value="">${copy.noSubjectsLabel}</option>`}
          ${hasRoutableSubjects && html`<option value="">${copy.autoSubjectLabel}</option>`}
          ${subjectOptions.map(
            (subject) => html`
      ...
      <div className="mb-3 rounded-lg border border-white/[0.06] bg-black/10">
        ...
        ${!channelsQuery.isLoading &&
        channels.length === 0 &&
        html`<div className="px-3 py-2 text-xs text-iron-500">${copy.emptyMessage}</div>`}
        ${channels.map(
          (channel) => html`
            <label
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        html`<p className="text-xs text-[var(--v2-positive-text)]">${copy.successMessage}</p>`}
...
function subjectOptionsForChannel(subjects = [], channel = {}) {
  ...
  return Array.from(bySubjectUserId.values()).sort(
    (left, right) =>
...
function slackChannelPickerCopy(action, t) {
  return {
    ...
    instructions: action?.instructions || t('channels.slackAccessInstructions'),
