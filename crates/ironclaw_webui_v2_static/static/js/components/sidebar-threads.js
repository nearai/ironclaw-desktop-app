    ...
    <div
      className=${cn(
        ...
        isActive
          ...
          : 'text-[var(--v2-text-muted)] hover:border-[var(--v2-panel-border)] hover:bg-[var(--v2-surface-soft)] hover:text-[var(--v2-text-strong)]'
      )}
    >
      <button
...
export function SidebarThreads({
   ...
   *     The active thread is no longer auto-pinned, so switching threads
   *     does not shuffle the list.
   *   - Recent: everything else, newest-first by updated_at || created_at.
   *
   * Per-thread state (running / needs-attention / failed) still renders its
   * dot wherever the thread lands; it no longer forces a thread into Pinned.
   *
