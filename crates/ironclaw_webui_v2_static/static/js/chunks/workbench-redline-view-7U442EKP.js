import{b as v,c as m}from"./chunk-IG4LZQG4.js";import"./chunk-NAT75VSJ.js";function A(n){return(typeof n=="string"?n:"").match(/\s+|\S+/g)||[]}function T(n){let o=[];for(let t of n){if(!t.text)continue;let r=o[o.length-1];r&&r.op===t.op?r.text+=t.text:o.push({op:t.op,text:t.text})}return o}function j(n,o){let t=n.length,r=o.length,s=Array.from({length:t+1},()=>new Int32Array(r+1));for(let l=t-1;l>=0;l--)for(let d=r-1;d>=0;d--)s[l][d]=n[l]===o[d]?s[l+1][d+1]+1:Math.max(s[l+1][d],s[l][d+1]);return s}function L(n,o){let t=A(n),r=A(o);if(t.length+r.length>4e3){let g=[];return typeof n=="string"&&n&&g.push({op:"delete",text:n}),typeof o=="string"&&o&&g.push({op:"insert",text:o}),g}let s=j(t,r),l=t.length,d=r.length,i=[],a=0,b=0;for(;a<l&&b<d;)t[a]===r[b]?(i.push({op:"equal",text:t[a]}),a++,b++):s[a+1][b]>=s[a][b+1]?(i.push({op:"delete",text:t[a]}),a++):(i.push({op:"insert",text:r[b]}),b++);for(;a<l;)i.push({op:"delete",text:t[a++]});for(;b<d;)i.push({op:"insert",text:r[b++]});return T(i)}function _(n,o){let t=String(n??"").match(/\S+/g)||[],r=String(o??"").match(/\S+/g)||[];return!t.length&&!r.length?1:!t.length||!r.length?0:2*j(t,r)[0][0]/(t.length+r.length)}var z=.4,B=2500;function S(n){return(typeof n=="string"?n:"").split(/\n+/).map(t=>t.trim()).filter(Boolean)}function C(n,o){let t=S(n),r=S(o),s=j(t,r),l=t.length,d=r.length,i=[],a=0,b=0;for(;a<l&&b<d;)t[a]===r[b]?(i.push({kind:"equal",before:t[a],after:r[b]}),a++,b++):s[a+1][b]>=s[a][b+1]?(i.push({kind:"del",before:t[a]}),a++):(i.push({kind:"ins",after:r[b]}),b++);for(;a<l;)i.push({kind:"del",before:t[a++]});for(;b<d;)i.push({kind:"ins",after:r[b++]});let g=(p,h)=>({kind:"modified",before:p,after:h,segments:L(p,h),changed:!0}),f=p=>({kind:"removed",before:p,after:"",segments:[{op:"delete",text:p}],changed:!0}),$=p=>({kind:"added",before:"",after:p,segments:[{op:"insert",text:p}],changed:!0}),x=[],w=0;for(;w<i.length;){if(i[w].kind==="equal"){let e=i[w];x.push({kind:"unchanged",before:e.before,after:e.after,segments:e.before?[{op:"equal",text:e.before}]:[],changed:!1}),w++;continue}let p=[],h=[];for(;w<i.length&&i[w].kind!=="equal";)i[w].kind==="del"?p.push(i[w].before):h.push(i[w].after),w++;let k=new Array(p.length).fill(!1),y=new Array(h.length).fill(-1);if(p.length*h.length<=B){let e=[];for(let c=0;c<p.length;c++)for(let u=0;u<h.length;u++){let R=_(p[c],h[u]);R>=z&&e.push({di:c,ii:u,sim:R})}e.sort((c,u)=>u.sim-c.sim||c.di-u.di||c.ii-u.ii);for(let c of e)k[c.di]||y[c.ii]!==-1||(k[c.di]=!0,y[c.ii]=c.di)}for(let e=0;e<p.length;e++)k[e]||x.push(f(p[e]));for(let e=0;e<h.length;e++)x.push(y[e]!==-1?g(p[y[e]],h[e]):$(h[e]))}return x.filter(Boolean).map((p,h)=>({id:`clause-${h}`,...p}))}function I(n,o){let t=Array.isArray(n)?n:[],r=o&&typeof o=="object"?o:{};return t.map(s=>{if(!s||typeof s!="object")return"";let l=r[s.id]==="reject"?s.before:s.after;return typeof l=="string"?l:""}).filter(s=>s.length>0).join(`
`)}var E={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"};function N(n){return String(n??"").replace(/[&<>"']/g,o=>E[o])}function M(n,{title:o="Redline"}={}){let t=Array.isArray(n)?n:[],r=N(o),s=t.map(l=>{if(!l||typeof l!="object")return"";let d=(Array.isArray(l.segments)?l.segments:[]).map(i=>{let a=N(i&&i.text);return i&&i.op==="insert"?`<ins>${a}</ins>`:i&&i.op==="delete"?`<del>${a}</del>`:a}).join("");return`<p class="c"><span class="k">${N(l.kind)}</span>${d}</p>`}).filter(Boolean).join(`
`);return`<!doctype html>
...
`;function D({segments:n}){let o=Array.isArray(n)?n:[];return m`<div className="wb13-rl-text">
    ${o.map((t,r)=>t.op==="insert"?m`<ins key=${r} className="wb13-rl-ins">${t.text}</ins>`:t.op==="delete"?m`<del key=${r} className="wb13-rl-del">${t.text}</del>`:m`<span key=${r}>${t.text}</span>`)}
  </div>`}function H({initialOriginal:n="",initialRevised:o=""}){let[t,r]=v.default.useState(n),[s,l]=v.default.useState(o),d=v.default.useMemo(()=>C(t,s),[t,s]),[i,a]=v.default.useState({}),[b,g]=v.default.useState(!1),f=d.filter(e=>e.changed).length,$=!!(t.trim()||s.trim()),x=e=>i[e]==="reject"?"reject":"accept",w=(e,c)=>{g(!1),a(u=>({...u,[e]:c}))},p=d.filter(e=>e.changed&&x(e.id)==="reject").length,h=v.default.useMemo(()=>I(d,i),[d,i]),k=async()=>{try{await navigator.clipboard.writeText(h),g(!0)}catch{g(!1)}},y=()=>{try{let e=new Blob([M(d,{title:"Redline"})],{type:"text/html"}),c=URL.createObjectURL(e),u=document.createElement("a");u.href=c,u.download="redline.html",document.body.appendChild(u),u.click(),u.remove(),setTimeout(()=>URL.revokeObjectURL(c),0)}catch{}};return m`
      ...
      <div className="wb13-page">
        <div className="wb13-wide">
          ...
          ${$?m`
                  ...
                  ${d.map(e=>{let c=x(e.id);return m`
                      ...
                      >
                        <div className="wb13-rl-head">
                          ...
                          ${e.changed?m`<span className="wb13-rl-decide">
                                <button
                                  type="button"
                                  className="is-accept"
              ...
              `:m`<div className="wb13-rl-empty" data-testid="workbench-redline-empty">
