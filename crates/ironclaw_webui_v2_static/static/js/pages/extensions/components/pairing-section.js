...
export function PairingSection({
  ...
  const handleApprove = React.useCallback((code) => pairing.approve({ code }), [pairing.approve]);
  ...
  return html`
    <div className="mt-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
      ...
      ${result &&
      !result.success &&
      html`<p className="mb-3 text-xs text-red-300">${result.message || pairingCopy.error}</p>`}
      ${error &&
