...
function summarizeTools(t, tools) {
  ...
  if (searches)
    segs.push(t(searches === 1 ? 'tool.runSearch' : 'tool.runSearches', { n: searches }));
  if (commands)
    segs.push(t(commands === 1 ? 'tool.runCommand' : 'tool.runCommands', { n: commands }));
...
function ToolActivityCard({ activity, nested = false }) {
  ...
  const row = html`
    <button
      ...
      className="v2-button flex w-full items-center gap-2.5 border-0 border-b border-[var(--v2-panel-border)] bg-transparent px-1 py-2 text-left text-sm"
    >
      <span className=${['h-2 w-2 shrink-0 rounded-full', dotClass].join(' ')} />
      <span className="shrink-0 text-[11px] font-medium text-[var(--v2-text-faint)]"
        >${STATUS_WORD[toolStatus] || 'run'}</span
      >
      <span
        className="shrink-0 truncate font-mono text-[13px] font-medium text-[var(--v2-text-strong)]"
        >${toolName}</span
      >
      ${toolDetail &&
      html`<span className="min-w-0 truncate font-mono text-xs text-[var(--v2-text-muted)]"
        >${toolDetail}</span
      >`}
      <span className="ml-auto flex shrink-0 items-center gap-2">
        ${toolStatus === 'running' &&
        !hasDuration &&
        html`<span className="text-[11px] text-[var(--v2-text-muted)]">…</span>`}
        ${hasDuration &&
        html`<span className="text-[11px] text-[var(--v2-text-muted)] tabular-nums"
  ...
  return html`
    <div className=${nested ? '' : 'flex gap-3'}>
      ...
      html`
        <div
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[var(--v2-panel-border)] bg-[var(--v2-surface-soft)] text-[var(--v2-text-muted)]"
        >
...
function ToolDetailPanel({
  ...
  const active =
    activeState && tabs.some((tab) => tab.id === activeState) ? activeState : tabs[0]?.id;
  React.useEffect(() => {
  ...
  if (tabs.length === 0) {
    return html`
      <div
        id=${controlsId}
        className="rounded-b-[10px] border-x border-b border-[var(--v2-panel-border)] bg-[var(--v2-surface-soft)] px-3 py-2 font-mono text-xs text-[var(--v2-text-muted)]"
      >
        ${t('tool.noDetail')}
      </div>
    `;
  ...
  return html`
    <div
      id=${controlsId}
      className="rounded-b-[10px] border-x border-b border-[var(--v2-panel-border)] bg-[var(--v2-surface-soft)]"
    >
      <div className="flex items-center gap-1 border-b border-[var(--v2-panel-border)] px-2 pt-1.5">
        ${tabs.map(
          (tab) => html`
            <button
              type="button"
              ...
              className=${[
                'v2-button rounded-t-[6px] px-2.5 py-1 text-[11px] font-medium',
                active === tab.id
        ...
        <span className="ml-auto px-1 py-1 text-[10px] text-[var(--v2-text-faint)] tabular-nums">
          ${toolStatus === 'error'
            ? t('tool.exitError')
            : toolStatus === 'running'
              ? t('tool.exitRunning')
      ...
      <div className="p-3 text-xs">
        ${active === 'details' &&
        html`<div className="whitespace-pre-wrap text-[var(--v2-text)]">${toolDetail}</div>`}
        ${active === 'params' &&
        html`<pre
          className="overflow-x-auto rounded-[6px] border border-[var(--v2-panel-border)] bg-[var(--v2-surface)] p-2 font-mono text-[var(--v2-text)]"
        >
${toolParameters}</pre
        >`}
        ...
        ${active === 'error' &&
        html`<pre
          className="overflow-x-auto whitespace-pre-wrap rounded-[6px] border border-[var(--v2-panel-border)] bg-[var(--v2-surface)] p-2 font-mono text-[var(--v2-danger-text)]"
        >
${toolError}</pre
        >`}
      </div>
...
function ToolResult({ text }) {
  ...
  if (/^data:image\/(?:png|jpe?g|gif|webp|bmp);/i.test(value)) {
    return html`<img
      src=${value}
      alt="Tool result"
      className="max-h-72 rounded-[10px] border border-[var(--v2-panel-border)] object-contain"
    />`;
  }
  ...
  if (Array.isArray(parsed) && parsed.length > 0 && parsed.every(isFlatRow)) {
    ...
    return html`
      <div className="overflow-x-auto rounded-[8px] border border-[var(--v2-panel-border)]">
            ...
            <tr>
              ${columns.map(
                (col) =>
                  html`<th
                    key=${col}
                    className="border-b border-[var(--v2-panel-border)] bg-[var(--v2-surface-soft)] px-2 py-1 font-semibold text-[var(--v2-text-strong)]"
                  >
                    ${col}
                  </th>`
              )}
  ...
  if (parsed !== undefined && typeof parsed === 'object') {
    return html`<pre
      className="overflow-x-auto whitespace-pre-wrap rounded-[6px] border border-[var(--v2-panel-border)] bg-[var(--v2-surface)] p-2 font-mono text-[var(--v2-text)]"
    >
