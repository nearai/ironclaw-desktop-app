...
function ProjectCard({ project, onOpen, t }) {
  return html`
    <article
      className="group rounded-xl border border-iron-700 bg-iron-800/60 p-5 hover:border-signal/30 hover:bg-iron-800/80"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3
            className="truncate font-serif text-2xl font-semibold tracking-[-0.03em] text-iron-100"
          >
            ${project.name}
          </h3>
              ...
              ${project.goals
                ...
                .map(
                  (goal, index) => html`
                    <span
                      key=${index}
                      className="rounded-full border border-iron-700 px-3 py-1 text-xs text-iron-200"
                    >
                      ${goal}
                    </span>
                  `
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-iron-700 bg-iron-950/55 p-3">
          ...
          <div className="mt-1 text-xs text-iron-300">
            ${t('projects.card.threadsToday', {
              count: compactCount(project.threads_today || 0, 'thread')
            })}
          </div>
        </div>
        <div className="rounded-2xl border border-iron-700 bg-iron-950/55 p-3">
          ...
          <div className="mt-1 text-xs text-iron-300">
            ${t('projects.card.failures24h', {
              count: compactCount(project.failures_24h || 0, 'failure')
            })}
          </div>
        </div>
      </div>
    ...
    <${Panel} className="overflow-hidden p-5 sm:p-6">
      <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-sm leading-6 text-iron-200">${t('projects.general.desc')}</p>
        </div>
        <div className="flex flex-wrap gap-3">
          <div
            className="rounded-2xl border border-iron-700 bg-iron-950/55 px-4 py-3 text-sm text-iron-200"
          >
...
export function ProjectsGrid({
  ...
  return html`
    <div className="space-y-5">
      ${defaultProject &&
      html`<${GeneralProjectCard} project=${defaultProject} onOpen=${onOpenProject} t=${t} />`}
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <${Button} onClick=${onCreateProject}
              >${isPreparingChat ? t('projects.preparingChat') : t('projects.newProject')}<//
            >
          </div>
        </div>
      <//>
      ...
      ${scopedProjects.length
        ...
        : html`
            ...
            >
              <${Button} onClick=${onCreateProject}
                >${isPreparingChat ? t('projects.preparingChat') : t('projects.startProject')}<//
              >
            <//>
          `}
    </div>
