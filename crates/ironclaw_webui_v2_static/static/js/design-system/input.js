...
const INPUT_SIZES = {
  sm: 'h-9 rounded-[7px] px-3 text-[12px]',
  md: 'h-[42px] rounded-[8px] px-3.5 text-[13px] md:h-[44px] md:px-4 md:text-sm',
  lg: 'h-[48px] rounded-[10px] px-4 text-base'
};
...
export function Textarea({ className = '', error = false, rows = 4, ...rest }) {
  return html`
    <textarea
      ...
      className=${cn(
        INPUT_BASE,
        'rounded-[8px] px-3.5 py-3 text-[13px] md:px-4 md:text-sm',
        'resize-y min-h-[80px]',
        error &&
          'border-[var(--v2-danger-text)] focus:ring-[color-mix(in_srgb,var(--v2-danger-text)_28%,transparent)]',
        className
