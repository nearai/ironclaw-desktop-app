...
registerPack('ja', {
  ...
  'login.heroSub':
    'トークンアクセスにより、ブラウザコンソールは同じゲートウェイランタイム、承認、ツール、スレッド状態に紐づきます。',
  ...
  'chat.emptyDesc':
    'メッセージ送信、ファイル添付、またはゲートウェイチェックを依頼できます。ワークスペースは承認とランタイム活動をターン中に可視化します。',
  ...
  'chat.noConversations':
    'まだ会話がありません。コンポーザーの提案からスレッドを開始してください。',
  ...
  'authGate.oauthWaiting':
    '認証が完了するまで待機しています... アクセスを承認したら、ポップアップ タブを閉じることができます。',
  ...
  'authGate.resolveFailedAfterTokenSaved':
    'トークンが保存されました。ブロックされた実行を再開できませんでした。再開してみてください。',
  ...
  'traceCommons.description':
    '提供した編集済みトレースで獲得したクレジット。アカウント単位で表示されます。',
  'traceCommons.emptyState':
    '未登録 — エージェントに Trace Commons の招待でオンボーディングを依頼してください。',
  ...
  'traceCommons.heldDescription':
    'プライバシーリスクが高いため保留中です。確認して送信を承認してください。',
  ...
  'traceCommons.note':
    '最終同期時点のローカル表示です — 正式なクレジット台帳はサーバー側にあります。確定クレジットはプライバシー審査、リプレイ/評価、重複チェック、後段の有用性スコアリングの後に変わる可能性があります。',
  'settings.back': '戻る',
  ...
  'settings.restartUnavailable':
    'ウェブ UI からの再起動はまだ利用できません。保留中の変更を適用するには、ゲートウェイプロセスを手動で再起動してください。',
  ...
  'onboarding.subtitle':
    '開始するにはAIプロバイダーを選択してください。後で設定から変更や追加が可能です。',
  ...
  'onboarding.providerOllamaDesc':
    '開いたモデルをローカルで実行します。 API キーは必要ありません。',
  ...
  'onboarding.nearaiLocalSso':
    'ブラウザでの NEAR AI サインイン（GitHub、Google、NEAR ウォレット）は localhost では利用できません。NEAR AI がローカルのコールバック URL を拒否するためです。代わりに NEAR AI の API キーを追加するか、公開 URL で実行してください。',
  ...
  'skills.noInstalledDesc':
    'スキルはエージェントにドメイン固有の指示を追加します。ClawHub レジストリからインストールするか、ワークスペースに SKILL.md を配置してください。',
  'skills.failedLoad': 'スキルの読み込みに失敗しました: {message}',
  'skills.import': 'スキルをインポート',
  'skills.importDesc':
    'HTTPS SKILL.md URL からインストールするか、SKILL.md コンテンツを直接貼り付けます。',
  'skills.name': 'スキル名',
  'skills.namePlaceholder': 'スキル名',
  'skills.url': 'HTTPS URL',
  'skills.urlHint':
    'SKILL.md またはサポートされているスキル バンドルへの直接 HTTPS リンクを使用します。',
  'skills.urlPlaceholder': 'https://example.com/SKILL.md',
  'skills.content': 'SKILL.md コンテンツ',
  'skills.contentHint':
    'ローカルまたはコピーしたスキル コンテンツをインポートする場合は、URL の代わりにこれを使用します。',
  ...
  'jobs.list.queueDesc':
    'タイトルまたはIDで検索し、実行に入って、ページを離れずにアクティブな作業を停止できます。',
  ...
  'jobs.list.empty.noMatchDesc':
    'より広い検索語を試すか、状態フィルターをリセットしてキューの残りを表示してください。',
  'jobs.list.empty.noJobsTitle': 'まだジョブがありません',
  'jobs.list.empty.noJobsDesc':
    'バックグラウンド作業、サンドボックス実行、オペレーター介入は、gatewayがジョブを作り始めるとここに表示されます。',
  ...
  'automations.delivery.explainer':
    'トリガーされた実行が完了したときに自動化の結果を配信する場所を選択します。',
  ...
  'automations.delivery.unpairedDesc':
    'DM 配信を有効にするには Slack アカウントを連携してください。',
  ...
  'automations.delivery.footnote':
    'DM に送信された承認リクエストは、Slack で {command} と返信して回答します。',
  ...
  'ext.registry.emptyDesc':
    '利用可能な拡張機能はすべてインストール済みか、レジストリが設定されていません。',
  ...
  'projects.creationDraft':
    '新しいプロジェクトを作成してください。次のための自律ワークスペースを設定したい: ',
  ...
  'job.noActivityDesc':
    'このジョブは、選択したフィルターに対して永続化されたイベントを記録していません。',
  ...
  'extensions.emptyInstalledDesc':
    'レジストリタブを参照して、WASMツール、チャネル、およびMCPサーバーをインストールしてください。',
  'extensions.emptyMcpTitle': 'MCPサーバーがありません',
  'extensions.emptyMcpDesc':
    'MCPサーバーは設定によりModel Context Protocolを介したツール機能をエージェントに拡張します。レジストリからインストールしてください。',
  'common.dismiss': '閉じる',
  'common.pin': 'ピン留め',
  'common.unpin': 'ピン留め解除',
  'common.remove': '削除',
