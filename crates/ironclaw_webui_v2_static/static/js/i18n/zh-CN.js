...
registerPack('zh-CN', {
  ...
  'traceCommons.loadFailed': '无法加载 Trace Commons 积分。',
  ...
  'traceCommons.note':
    '截至最近同步的本地视图 — 权威积分账本在服务器端。最终积分可能在隐私审核、重放/评估、去重检查和后续效用评分后发生变化。',
  'settings.back': '返回',
  ...
  'onboarding.nearaiLocalSso':
    '本地环境（localhost）不支持 NEAR AI 浏览器登录（GitHub、Google、NEAR 钱包）——NEAR AI 会拒绝本地回调地址。请改用 NEAR AI API key，或在公网地址下运行。',
  ...
  'skills.noInstalledDesc':
    '技能为代理提供特定领域的指令。从 ClawHub 注册表安装或在工作区放置 SKILL.md 文件。',
  'skills.failedLoad': '加载技能失败：{message}',
  'skills.import': '安装技能',
  'skills.importDesc':
    '通过一个 HTTPS 协议的 SKILL.md 链接以安装，或者直接填入原 SKILL.md 文本内容。',
  'skills.name': '技能名称',
  ...
  'jobs.list.queueDesc':
    '按标题或 ID 搜索，进入一次运行，并在不离开页面的情况下停止正在进行的工作。',
  ...
  'jobs.list.empty.noJobsDesc':
    '当 gateway 开始创建任务后，后台工作、sandbox 运行以及操作员干预都会显示在这里。',
  ...
  'automations.filterLabel': '按自动化状态类别过滤设定',
  ...
  'automations.summary.scheduledDetail':
    '列出了系统当前为这位 AI 代理可见的所有正在进行的自动化项目与计划清单调度项。',
  ...
  'automations.empty.noneDescription':
    '目前的配置上并尚未见到代理会在此具备预期内正活跃要呈现出现的计划执行项或调度运作中自动内容。您可以添加并再查看此列表情况情况是否产生了增加或者有变化！',
  ...
  'projects.scoped.onlyGeneralDesc':
    '当工作值得拥有独立的任务、文件、组件与长期上下文时，创建一个命名项目。',
  ...
  'projects.empty.noneDesc':
    '当助手创建持久化工作区后，项目就会出现。你可以从聊天开始，并让 IronClaw 为持续工作创建一个范围项目。',
  ...
  'pairing.error':
    '所尝试运行的上述匹配过程遭意外遇到并遇到发生了不可知失败原因。麻烦可稍后检查或者再从头重做配对要求步骤和密码输入信息。',
  'pairing.none': '没有待处理的配对请求。',
  'pairing.slackTitle': '当前正建立关于 Slack 组织帐户联通方式并提供设定配置关联。',
  'pairing.slackInstructions':
    '前往给本处 Slack 服务应用程式直接发起讯息交流动作过程，并且将生成的代码取回复写和抄填入到底下的方框位置内部：',
  'pairing.slackPlaceholder':
    '此处开始录入填入所需来自上述的 Slack 配对确认有效验证代码密码输入情况：',
  'pairing.connect': '点击通过去连接验证并校验这种配对工作',
  'pairing.slackSuccess':
    '针对所提供需要的 Slack 系统帐户，应用已被设置关联成功地进行接入且链接上系统上了！',
  'pairing.slackError':
    '被输进入过的此类 Slack 服务程序对应代码由于是不存在其生效期的或者存在任何拼写的其它异常的不可行的错误原因，所以已出现无法响应验证了。您得需要核实后可重试一遍此处的绑定方法！',
  ...
  'extensions.emptyInstalledDesc':
    '浏览扩展注册表去发现并安装 WASM 工具、服务通道 和各种 MCP 协议服务服务器。',
  'extensions.emptyMcpTitle': '暂无 MCP 代理服务器',
  'extensions.emptyMcpDesc':
    'MCP 服务器可通过模型上下文协议（Model Context Protocol）扩展代理与外界交互能力。从应用注册表（Registry）处可获取与安装。',
  'common.dismiss': '关闭',
  'common.pin': '置顶',
  'common.unpin': '取消置顶',
  'common.remove': '移除',
