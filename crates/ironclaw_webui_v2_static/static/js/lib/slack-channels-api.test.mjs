...
test('normalizeSlackChannelIds trims, dedupes, drops blanks, and sorts', () => {
  assert.deepEqual(normalizeSlackChannelIds([' C0OPS ', '', 'C0ENG', 'C0OPS']), ['C0ENG', 'C0OPS']);
});
...
test('saveSlackAllowedChannels sends explicit selected subjects when present', async () => {
  ...
  await saveSlackAllowedChannels([{ channel_id: 'C0OPS', subject_user_id: 'user:ops-team-agent' }]);
