...
- **Next:** (a) relabel "Needs a decision" → briefing's "Needs you"; (b) build the "Worth weighing in" radar (workbench-radar.js foundation shipped, #7-gated for the long read-everything turn); (c) deferred dead 'upcoming' config-chain cleanup (FEED_GROUPS + feed-api map + shell dot + TRIAGE_EXCLUDED).
