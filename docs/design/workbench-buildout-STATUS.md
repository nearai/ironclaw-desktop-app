...
- **LIVE: latency SOLVED, render NOT.** One synthesis thread, converged ~30s with valid JSON (needsYou=1) — the #7 wall is sidestepped. BUT the briefing area renders nothing live (no deterministic, no loading, no rich; DOM wrap = command/decisions/triage, no wb13-brief). runBriefing fired (thread exists) so setBriefing ran — yet nothing renders. Works in the mocked a11y test → a LIVE state/render bug, opaque to eval. Evidence: docs/design/evidence/deterministic-radar-live-*.md.
- **DIAGNOSED (instrumented):** the render PATH works — setBriefing(det) commits, HomeView re-renders, WorkbenchBriefing renders WITH det (hasBriefing:true confirmed in console). THEN the briefing reverts to NULL with NO dismissBriefing call → only the useState initializer nulls it → **WorkbenchPage is remounting** (resetting state) shortly after the brief is set. Separately, this session's live connectors FAILED ("6 sources could not be read") so synthesis returns null regardless (environmental, transient). Evidence: docs/design/evidence/render-hiccup-diagnosed-*.md.
- **FIXED + LIVE-VERIFIED.** Root cause: RequireAuth (app/app.js) returned <AuthLoading/> whenever auth.isChecking — so a BACKGROUND token re-exchange (queryClient.clear()) remounted EVERY page, resetting WorkbenchPage's useState (briefing->null). Fix (1 line): \`if (auth.isChecking && !auth.isAuthenticated)\` — only block on the initial check; keep an authenticated app mounted through background re-checks. The rich briefing now RENDERS + PERSISTS live: "1 awaiting your reply · 4 weekly signals", NEEDS YOU (Alex Scharrer, synthesized CLO-scoped context), THIS WEEK (4 calendar items), v13 fidelity. Screenshot evidence: docs/design/evidence/briefing-renders-live (commit 76f2623). Full gate green (static 882, a11y 140, design, smoke). — was: confirm the remount (mount/unmount useEffect log on WorkbenchPage); if confirmed, lift the briefing state above the remount boundary OR stop the remount (find the changing key / error-boundary reset). Re-test on a HEALTHY connector boot. — was: INSTRUMENT the render path (trigger branch / runBriefing setBriefing(det) / HomeView briefing.kind / lazy WorkbenchBrief import), re-test live, read logs, fix. Candidate fixes: un-lazy WorkbenchBrief (+ check cold-start <401) / visible Suspense fallback / briefingTokenRef guard / briefingReadsPending loading-strand. THEN (B) auto-run on open.## Synthesis SPLIT into two parallel turns — the live-latency fix (`8afeecf`)
...
- **LIVE result:** the split fires both turns; the **needsYou turn converged (~30s, valid JSON)**, but the radar turn (~1.8KB) still overran (>58s) — even small generative turns are slow on z-ai/glm-5.2 (#7). Also found a design flaw (Promise.all waits for the SLOW turn) + a live render hiccup (neither brief showed; no console error). Evidence: docs/design/evidence/synthesis-split-live-*.md.
- **Next (scheduled):** (a) make the split PROGRESSIVE — render needsYou as soon as turn A resolves; don't await the radar. (b) Make worthWeighingIn / thisWeek / bestTimes **DETERMINISTIC** (workbench-radar.js already scopes domain+channels — derive "worth weighing in" by scanning slack signals for domain-trigger matches I wasn't tagged on; thisWeek from calendar; bestTimes from reply windows) — this REMOVES the slow radar LLM turn entirely, leaving ONE fast LLM turn (needsYou ~30s). (c) Debug the live non-render (briefingTokenRef guard / Suspense). Root dependency: gateway #7 latency.
...
- **Live-verify BLOCKED (environmental, not code):** deterministic briefing rendered live; the rich brief did NOT, because the synthesis turn returned no assistant reply in 60s. Isolated: a trivial "reply OK" probe ALSO never replied and GET /llm/providers flipped 200→**404** mid-session — the gateway LLM provider dropped, so NO turn converges (the #7 issue / flaky NEAR AI provider this boot). Wiring is proven by the 2 mocked-turn tests. Evidence: docs/design/evidence/briefing-wired-3A-20260623.md.
- **Reprioritized next (unblocked first):** (C) remove the ops-dump TriageSection from the home main column [pure UI, testable via mocks, directly addresses the "can't see the rest" noise] + **React.lazy WorkbenchBrief** (reclaim the cold-start budget, 400.3→~396). THEN (B) auto-run on open (needs a page-load test opt-out). Re-verify the live rich brief once the gateway turn path is healthy; consider a tighter prompt + faster synthesis model to make the turn cheap (tie-in #7).
...
- Additive + inert this commit (nothing imports it yet — like the engine); the wiring is step 3. Full gate GREEN: static 879 (+3), a11y 138, design DT-1..6, smoke, cold-start 396.2<401.
- **Step 3/3 (next loop iteration):** wire synthesizeBriefing into `runBriefing` (synthesis-first, deterministic fallback) behind the briefing trigger, render WorkbenchBrief on rich success, then auto-run on open + remove the ops-dump TriageSection; spec test with a mocked synthesis turn; live screenshot. **Open input to resolve:** the user-profile source (name/title→domain, channel allowlist) for the radar — for v1 degrade safely to no-radar when absent; wire a real profile (Slack profile read or settings) after.
...
- **Re-homed, nothing dropped blind:** the INITIATED-not-Ready guarantee is already the source-inspector test's subject (744); 6 briefing/source tests used the strip only as a "connectors live" liveness proxy (each already polls connectorReadRequests / asserts briefing content) so the redundant lines were removed; 2 interaction tests swapped the proxy for `coldstart count 0`.
- Full gate GREEN: static 869/0, a11y 138, design DT-1..6, smoke, cold-start 396.0<401. Live DOM: sources-ready 0 / arrived 0 / upcoming 0 / decisions 1 / coldstart 0; screenshot confirms v13 fidelity + clean home. Evidence: docs/design/evidence/home-cut-connector-health-20260623.md.
- **The three-surface home cruft cut the user named is COMPLETE** (Upcoming → Arrived → connector-health). Home is now: Ask bar + suggestion chips + "Needs a decision" cards + the rail.
- **Next:** (a) relabel "Needs a decision" → briefing's "Needs you"; (b) build the "Worth weighing in" radar (workbench-radar.js foundation shipped, #7-gated for the long read-everything turn).
- **CORRECTION (do NOT cut 'upcoming' config):** earlier flagged the 'upcoming' chain (WORKBENCH_FEED_GROUPS + feed-api map + shell dot + TRIAGE_EXCLUDED) as "dead config to cut." It is NOT dead — it is a **live backend-feed rail pathway** (server-advertised calendar/upcoming feed items; feed-kind descriptor + text-classifier in workbench-feed-api.js; rendered as `workbench-rail-upcoming`; covered by workbench-feed-api.test.mjs). Only the *connector-derived* calendar moved to the Calendar tab. Removing it would break the feed-api test and drop a feature. Leave it.
