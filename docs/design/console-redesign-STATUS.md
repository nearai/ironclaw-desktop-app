...
- [ ] **C2b — center Triage cockpit**: make the center read as Direction B's "Triage" — a header (title + "N need you · M handled overnight") + filter pills (All/Decisions/Replies/Blocked) that filter the status sections. Consolidate the stacked center (briefing/decisions/triage) carefully (many specs depend on the current sections — update them in-chunk).
