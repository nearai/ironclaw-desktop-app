...
- Static bundle-size gate: `npm run check:static-bundle`
  ...
  - Current gzipped tracked assets: cold start 346.2 KB / 400 KB; `main.bundle.js` 310.8 KB / 350 KB; boot vendor 22.3 KB / 35 KB; lazy code highlighting 52.0 KB / 60 KB; lazy diagrams 858.5 KB / 950 KB; document/PDF lazy assets 516.3 KB / 600 KB; OCR lazy assets 2501.5 KB / 2800 KB; all tracked assets 4274.5 KB / 4900 KB.
...
- Static JS tests: `npm run test:static` -> 336 passed.
...
- Static accessibility gate: `npm run test:a11y-static`
  - Status: passed.
  - Full static Playwright project now runs 28 rendered tests: 15 axe surfaces, 3 attachment ingress proofs, 2 connector proofs, 6 keyboard/approval flows, 1 hostile markdown geometry proof, and 1 Mermaid diagram proof.
...
| Connections registry | YELLOW | Gmail, Google Calendar, Slack, and Notion appear as easy primary actions with honest install/setup guidance. Live probe found no route-contract violations, and rendered connector tests now prove registry clicks keep slash-prefixed refs out of lifecycle URLs and explicit chat setup prompts deep-link to Connections instead of becoming dead model prompts. | Keep UI credential-proof guard because backend raw activate still reports authenticated without credential proof. |
...
| Generated diagrams | GREEN/YELLOW | Mermaid fences now render as first-class diagram cards in chat, stay off the cold-start path, require explicit user click, initialize with strict Mermaid security, sanitize returned SVG, and are covered by both source tests and a rendered static Playwright proof. Exports now preserve labeled Mermaid source across MD/HTML/JSON/PDF/DOCX instead of dropping it into anonymous fences. | Keep. Full VIZ-3 is still YELLOW until DOCX/PDF/HTML can embed the rendered diagram image from the same render path, not only the source. |
