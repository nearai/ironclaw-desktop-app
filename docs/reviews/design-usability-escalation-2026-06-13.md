...
- Static bundle-size gate: `npm run check:static-bundle`
  ...
  - Current gzipped tracked assets: cold start 345.0 KB / 400 KB; `main.bundle.js` 309.6 KB / 350 KB; boot vendor 22.3 KB / 35 KB; lazy code highlighting 52.0 KB / 60 KB; lazy diagrams 858.5 KB / 950 KB; document/PDF lazy assets 516.3 KB / 600 KB; OCR lazy assets 2501.5 KB / 2800 KB; all tracked assets 4273.3 KB / 4900 KB.
...
- Static JS tests: `npm run test:static` -> 332 passed.
...
- Static accessibility gate: `npm run test:a11y-static`
  - Status: passed.
  - Full static Playwright project now runs 27 rendered tests: 15 axe surfaces, 3 attachment ingress proofs, 1 connector lifecycle proof, 6 keyboard/approval flows, 1 hostile markdown geometry proof, and 1 Mermaid diagram proof.
...
| Generated diagrams | GREEN | Mermaid fences now render as first-class diagram cards in chat, stay off the cold-start path, require explicit user click, initialize with strict Mermaid security, sanitize returned SVG, and are covered by both source tests and a rendered static Playwright proof. | Keep. Next visualization push is VIZ-3 export parity: the same diagram must export losslessly as an image inside DOCX/PDF/HTML/MD/JSON workflows. |
