...
| **A11Y-3** | Keyboard nav + focus management — focus order, modal/gate traps, ⌘⏎/Esc on gates, visible focus rings | M | — | components | tab-through reaches every control; gates resolve via keyboard |
...
| id | task | effort | deps | owned lane | acceptance |
|----|------|--------|------|-----------|-----------|
| **VIZ-1** | V0 — SVG `max-width` + table-overflow CSS hardening | S | — | `app.css`, markdown CSS | wide SVG/table never breaks layout |
| **VIZ-2** | V1 — **Mermaid** (lazy vendored v11, on-click render, sanitized) — model emits mermaid fences unprompted | M | VIZ-1 | `markdown-renderer.js`, `vendor/mermaid` | fenced `mermaid` renders on click; sanitized; lazy-loaded |
| **VIZ-3** | **render==export law** — same markdown AST drives screen + every export; real lists/links/tables in DOCX; whole-conversation export; diagram image export (builds on the byte-accurate PDF/DOCX already shipped) | M | VIZ-2 | `work-product-export.js`, `markdown` lib | byte-level parity test screen↔export across lists/links/tables/math |
