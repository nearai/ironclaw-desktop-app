...
| **CHAT-2** | File **artifact chip** with preview/download — extend chip-click document preview to agent-**generated** work products, same chip grammar | M | — | `attachment-preview.js`, `artifact-chip.js` | IN PROGRESS: generated markdown/document assistant work now renders with an explicit `Generated document` artifact header, copy, Save to Work, and scoped export controls; rendered static smoke proves artifact-header export to MD/HTML/JSON/PDF/DOCX. Save to Work now opens a hidden `/work` reader that reloads the saved local artifact, preserves the thread link, and exposes copy/MD/DOCX/PDF/HTML/JSON actions (`work-product-static.spec.ts`). Remaining acceptance is native generated binary chips (DOCX/PDF/XLSX/PPTX as files) → preview → save. |
