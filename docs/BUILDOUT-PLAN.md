...
6. **Visualization/export parity is incomplete** — Mermaid now renders lazily and safely in
   ...
   generated-document artifact headers are now explicit in chat; remaining competitor chat
   patterns (native binary artifact chips, receipt cards, tool rows, bubble-less) are still
   incomplete.
