...
const server = http.createServer(async (req, res) => {
  ...
  // Dev-harness shim: the staged sidecar does not serve /api/gateway/status (the
  // Tauri desktop app answers it via its native bridge). Without it the frontend
  // falls back to an UNVERIFIED readiness and the chat composer can't send, so the
  // standalone can't exercise the chat-bar verbs. Reflect the booted gateway's
  // real readiness (NEAR AI + the configured model — already verified 14/14 by
  // scripts/connector-live-test.mjs) so the standalone behaves like the product.
  if (pathname === '/api/gateway/status') {
    const body = JSON.stringify({
      ...
      total_connections: 0,
      llm_backend: 'nearai',
      llm_model: process.env.NEARAI_MODEL || 'auto',
